...
public class AudioManager : MonoBehaviour {
    ...
    private void Awake() {
        if (instance == null) {
            ...
            // Caso ja tenha um Singleton, destroi a instancia atual
            Destroy(gameObject);
            return;
        }
    ...
    /// <param name="forceReplay">Caso seja True a música irá reiniciar, caso seja False só dará play se já não estiver tocando</param>
    public void PlayMusic (string name, bool forceReplay = false) {
        Audio result = GetMusic(name);

        if (result != null) {
            if (forceReplay || !result.source.isPlaying) {
                StopAllMusic();
                result.source.Play();
            }
        } else {
